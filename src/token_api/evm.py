...
from .types import (
    # Enums
    NetworkId, TokenStandard, ActivityType, OrderDirection, OrderBy, 
    Interval, Protocol,
    
    # Response types
    NFTOwnershipsResponse, NFTCollectionsResponse, NFTItemsResponse,
    NFTActivitiesResponse, NFTHoldersResponse, NFTSalesResponse,
    BalancesResponse, TransfersResponse, TokensResponse, TokenHoldersResponse,
    SwapsResponse, PoolsResponse, OHLCResponse, HistoricalBalancesResponse
)
...
class EVMTokenAPI(BaseTokenAPI):
    ...
    async def get_token_holders(
        self,
        contract: str,
        order_by: Union[OrderBy, str] = OrderBy.VALUE,
        order_direction: Union[OrderDirection, str] = OrderDirection.DESC,
        limit: int = 10,
        page: int = 1
    ...
    async def get_swaps(
        ...
        response = await self.manager.get(
            f"{self.base_url}/swaps/evm",
            headers=self._headers,
            params=params,
            expected_type=SwapsResponse
        )
    ...
    async def get_pools(
        ...
        response = await self.manager.get(
            f"{self.base_url}/pools/evm",
            headers=self._headers,
            params=params,
            expected_type=PoolsResponse
        )
        return response
